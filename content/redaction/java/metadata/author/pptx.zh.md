...
draft: false
path: "zh/redaction/java/metadata/author/pptx"
...
ad_description: "从 Java 中的 PPTX 元数据中编辑 Author Name"
############################# Head ############################
head_title: "从 Java 的 PPTX 元数据中删除或更新作者姓名"
head_description: "GroupDocs.Redaction s 提供了灵活的 API，允许使用过滤器搜索和替换/删除任何元数据属性。"

############################# Header ############################
title: "从 Java 的 PPTX 元数据中删除或更新作者姓名"
...
steps:
    enable: true
    block:
    - title_left: "编辑 Java 中的 PPTX 元数据"
      content_left: |
        以下代码允许您搜索和编辑 PPTX 文档中的敏感数据。你可以通过设置过滤器来设置编辑范围，例如设置为 metadataFilter.Author。它将撤消所有元数据项中的正则表达式匹配项，但 “Author” 属性除外： 

      title_right: "如何编辑 PPTX 元数据"
      content_right: |
        * 创建 [Redactor](https://apireference.groupdocs.com/redaction/java/com.groupdocs.redaction/Redactor) 类的实例并上传 PPTX 文件
