...
draft: false
path: "ja/redaction/net/text/ppt/"
...
############################# Header ############################
title: ".NET Core AppsのC＃を介してPPT ファイルを編集します"
description: "OfficeとOpenOfficeのドキュメント、スプレッドシート、プレゼンテーションのテキストを検索して置換し、Windows、Linux、macOSでは PPT"
...
steps:
    enable: true
    block:
    - title_left: "PPTからC＃経由で正確なテキストを編集"
      content_left: |
        [GroupDocs.Redaction](/redaction/net/) を使用すると、.NET開発者はいくつかの簡単な手順でPPTファイル編集機能を簡単に追加できます。

        * [Redactor](https://apireference.groupdocs.com/redaction/net/groupdocs.redaction/redactor) クラスのインスタンスを作成し、PPTファイルをロードします
