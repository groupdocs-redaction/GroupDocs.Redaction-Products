...
draft: false
path: "zh/redaction/net/metadata/createdtime/pps"
...
about:
    ...
    title: "什么是元数据编辑？"
    content: |
        所有文件格式，包括 PDF、DOC、DOCX、PPT、PPTX、XLS、XLSX 等，都有一些元数据属性。这些属性包括作者姓名、类别、公司名称、评论、创建时间、上次更新等等。编辑是指从电子文档中删除不需要或机密的信息。有时您需要完全删除不需要的元数据字段，或者可能需要更新它们的值。还有一些与文件相关的隐藏数据，可以使用各种工具和技术进行查看。在很多情况下，您不希望任何人访问这些数据。使用 GroupDocs.Redaction API，您可以对这些元数据属性中的任何一个应用元数据编辑。您可以通过筛选所需的元数据来更改或删除它们。在本指南中，我们将解释如何从 C# 的 PPS 文档中编辑 createdtime 元数据。

############################# content ############################
steps:
    enable: true
    block:
    - title_left: "从 C# 的 PPS 中移除元数据"
      content_left: |
        你可以使用 eraseMetadataredAction 类将 PPS 文档中的所有或特定元数据替换为空值。以下代码显示了如何筛选然后从 PPS 文档中删除元数据属性。以下示例将该文档的所有属性都清空：
        

      title_right: "清除 PPS 文档的元数据"
      content_right: |
        * 创建 [Redactor](https://apireference.groupdocs.com/redaction/net/groupdocs.redaction/redactor) 类的实例并上传 PPS 文件
    ...
    - title_left: "编辑 C# 中的 PPS 元数据"
      content_left: |
        以下代码允许您搜索和编辑 PPS 文档中的敏感数据。你可以通过设置过滤器来设置密文范围，例如设置为 metadataFilter.createdTime。-它会撤消所有元数据项中的正则表达式匹配项，但 “Createdtime” 属性除外：
        
        
      title_right: "如何编辑 PPS 元数据"
      content_right: |
        * 创建 [Redactor](https://apireference.groupdocs.com/redaction/net/groupdocs.redaction/redactor) 类的实例并上传 PPS 文件
